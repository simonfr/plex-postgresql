...
## 🎉 Latest Release: v0.9.25

**Memory telemetry and stability:** added optional per-subsystem allocation tracking and hardened cleanup paths.

- ✅ **Added:** optional memory telemetry (`PLEX_PG_MEM_TELEMETRY=1`) — logs bind, blob, column cache, and sweep stats every 60s
- ✅ **Fixed:** (v0.9.24) statement teardown frees all `MAX_PARAMS` slots to prevent stale heap growth
- ✅ **Added:** (v0.9.24) leak regression tests with macOS `leaks` tool

[📥 Download v0.9.25](https://github.com/cgnl/plex-postgresql/releases/tag/v0.9.25) | [📋 Full Release Notes](https://github.com/cgnl/plex-postgresql/releases/tag/v0.9.25)
...
Pull requests and `main` pushes run `.github/workflows/ci.yml` (script validation + Linux amd64 build check + **781 unit tests**).
...
**macOS:**
```bash
curl -L https://github.com/cgnl/plex-postgresql/releases/download/v0.9.25/plex-postgresql-v0.9.25-macos.zip \
...
**Linux (x86_64):**
```bash
sudo curl -L https://github.com/cgnl/plex-postgresql/releases/download/v0.9.25/plex-postgresql-v0.9.25-linux.zip \
