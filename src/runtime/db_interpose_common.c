...
#define EXC_QUERY_RING_SIZE 24
#define EXC_QUERY_MAX_LEN 320
#define EXC_PHASE_RING_SIZE 48
#define EXC_PHASE_MAX_LEN 32

static char exc_query_ring[EXC_QUERY_RING_SIZE][EXC_QUERY_MAX_LEN];
static int exc_query_ring_next = 0;
static pthread_mutex_t exc_query_ring_mutex = PTHREAD_MUTEX_INITIALIZER;

typedef struct {
    char phase[EXC_PHASE_MAX_LEN];
    char sql[EXC_QUERY_MAX_LEN];
    void *stmt;
    void *db;
    unsigned long tid;
} exc_phase_entry_t;

static exc_phase_entry_t exc_phase_ring[EXC_PHASE_RING_SIZE];
static int exc_phase_ring_next = 0;
static pthread_mutex_t exc_phase_ring_mutex = PTHREAD_MUTEX_INITIALIZER;
...
int (*shim_sqlite3_prepare_v2)(sqlite3*, const char*, int, sqlite3_stmt**, const char**) = NULL;
const char* (*shim_sqlite3_errmsg)(sqlite3*) = NULL;
int (*shim_sqlite3_errcode)(sqlite3*) = NULL;
...
int is_library_db_path(const char *path) {
    return rust_is_library_or_blobs_db_path(path);
}

// Helper: check if path is for blobs.db
int is_blobs_db_path(const char *path) {
    return rust_is_blobs_db_path(path);
}
...
void common_load_sqlite_symbols(void *handle) {
    ...
    if (!shim_sqlite3_prepare_v2) shim_sqlite3_prepare_v2 = orig_sqlite3_prepare_v2;
    if (!shim_sqlite3_errmsg) shim_sqlite3_errmsg = orig_sqlite3_errmsg;
    if (!shim_sqlite3_errcode) shim_sqlite3_errcode = orig_sqlite3_errcode;
...
char* print_exception_info(const char *type_name,
    ...
    char what_buf[193];
    what_buf[0] = '\0';
    int has_what = pg_exception_extract_what(thrown_exception,
                                             tinfo,
                                             what_buf,
                                             sizeof(what_buf));
    if (has_what) {
        char what_display[73];
        snprintf(what_display, sizeof(what_display), "%.72s", what_buf);
        fprintf(stderr, "║ What: %-72s ║\n", what_display);
    } else {
        fprintf(stderr, "║ What: %-72s ║\n", "(unavailable at throw site)");
    }
    ...
    LOG_ERROR("EXCEPTION #%d [%s]: what='%s' shim=%s tls_shim=%s col=%ld val=%ld",
              count, readable_name,
              has_what ? what_buf : "",
...
int common_handle_exception(void *thrown_exception, void *tinfo,
    ...
    *in_handler_flag = 1;
    ...
    const char *verbose_env = getenv("PLEX_PG_EXCEPTION_VERBOSE");
    int verbose_exceptions = (verbose_env && strcmp(verbose_env, "0") != 0);
    const char *nonshim_db_env = getenv("PLEX_PG_EXCEPTION_LOG_NONSHIM_DB");
    int log_nonshim_db = (nonshim_db_env && strcmp(nonshim_db_env, "0") != 0);

    // Only treat known DB exceptions as critical by default.
    int is_db_exception = (type_name &&
                           (strstr(type_name, "N2DB") ||
                            strstr(type_name, "DB9Exception") ||
                            strstr(type_name, "DB::Exception")));

    int this_thread_used_shim =
        (tls_column_type_calls > 0 || tls_value_type_calls > 0 || tls_last_query != NULL);

    int should_log = verbose_exceptions ||
    ...
    if (should_log) {
        ...
        char *demangled = print_exception_info(type_name,
                                               total_count,
                                               thrown_exception,
                                               tinfo);

        if (is_db_exception) {
            pg_exception_dump_recent_queries();
            pg_exception_dump_recent_phases();
        }
...
const char* pg_exception_get_last_query(void) {
    return last_query_being_processed;
}

const char* pg_exception_get_last_column(void) {
    return last_column_being_accessed;
}
...
void pg_exception_note_query(const char *sql) {
    ...
    pthread_mutex_lock(&exc_query_ring_mutex);
    snprintf(exc_query_ring[exc_query_ring_next], EXC_QUERY_MAX_LEN, "%.319s", sql);
    exc_query_ring_next = (exc_query_ring_next + 1) % EXC_QUERY_RING_SIZE;
    pthread_mutex_unlock(&exc_query_ring_mutex);
}

void pg_exception_dump_recent_queries(void) {
    pthread_mutex_lock(&exc_query_ring_mutex);
    fprintf(stderr, "[EXC_CONTEXT] Recent SQL (oldest -> newest):\n");
    for (int i = 0; i < EXC_QUERY_RING_SIZE; i++) {
        int idx = (exc_query_ring_next + i) % EXC_QUERY_RING_SIZE;
        if (exc_query_ring[idx][0]) {
            fprintf(stderr, "[EXC_CONTEXT]   [%02d] %.319s\n", i, exc_query_ring[idx]);
        }
    }
    fflush(stderr);
    pthread_mutex_unlock(&exc_query_ring_mutex);
...
void pg_exception_note_phase(const char *phase,
                             ...
                             const void *stmt,
                             const void *db) {
    pthread_mutex_lock(&exc_phase_ring_mutex);

    exc_phase_entry_t *slot = &exc_phase_ring[exc_phase_ring_next];
    snprintf(slot->phase, sizeof(slot->phase), "%.31s", phase ? phase : "unknown");
    if (sql && sql[0]) {
        snprintf(slot->sql, sizeof(slot->sql), "%.319s", sql);
    } else {
        slot->sql[0] = '\0';
    }
    slot->stmt = (void *)stmt;
    slot->db = (void *)db;
    slot->tid = (unsigned long)pthread_self();

    exc_phase_ring_next = (exc_phase_ring_next + 1) % EXC_PHASE_RING_SIZE;

    pthread_mutex_unlock(&exc_phase_ring_mutex);
}

void pg_exception_dump_recent_phases(void) {
    pthread_mutex_lock(&exc_phase_ring_mutex);

    fprintf(stderr, "[EXC_CONTEXT] Recent phases (oldest -> newest):\n");
    for (int i = 0; i < EXC_PHASE_RING_SIZE; i++) {
        int idx = (exc_phase_ring_next + i) % EXC_PHASE_RING_SIZE;
        exc_phase_entry_t *e = &exc_phase_ring[idx];
        if (!e->phase[0]) {
            continue;
        }
        ...
        if (e->sql[0]) {
            fprintf(stderr,
                    "[EXC_CONTEXT]   [%02d] phase=%s tid=0x%lx stmt=%p db=%p sql=%.200s\n",
                    i, e->phase, e->tid, e->stmt, e->db, e->sql);
        } else {
            fprintf(stderr,
                    "[EXC_CONTEXT]   [%02d] phase=%s tid=0x%lx stmt=%p db=%p\n",
                    i, e->phase, e->tid, e->stmt, e->db);
        }
    }
