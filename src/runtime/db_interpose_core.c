...
static void setup_fishhook_rebindings(void) {
    ...
    struct rebinding rebindings[] = {
        ...
        // C++ exception interception is configured separately at runtime.
    };
...
static void setup_exception_rebinding_if_enabled(void) {
    const char *enable = getenv("PLEX_PG_ENABLE_EXCEPTION_CATCHER");
    if (!enable || strcmp(enable, "0") == 0) {
        return;
    }

    struct rebinding exception_rebinding = {
        "__cxa_throw", (void*)my_cxa_throw, (void**)&orig_cxa_throw
    };

    int rc = rebind_symbols(&exception_rebinding, 1);
    if (rc == 0) {
        fprintf(stderr, "[SHIM_INIT] Exception catcher enabled (__cxa_throw hooked)\n");
        pg_exception_install_terminate_logger();
        fprintf(stderr, "[SHIM_INIT] Exception terminate logger enabled\n");
    } else {
        fprintf(stderr, "[SHIM_INIT] WARNING: failed to hook __cxa_throw (rc=%d)\n", rc);
    }
    fflush(stderr);
}
...
static void load_sqlite_fallback(void) {
    ...
    if (sqlite_handle && (!shim_sqlite3_prepare_v2 || !orig_sqlite3_prepare_v2)) {
...
void ensure_real_sqlite_loaded(void) {
    if (shim_sqlite3_prepare_v2) return;
    ...
    if (sqlite_handle) {
        shim_sqlite3_prepare_v2 = dlsym(sqlite_handle, "sqlite3_prepare_v2");
        shim_sqlite3_errmsg = dlsym(sqlite_handle, "sqlite3_errmsg");
        shim_sqlite3_errcode = dlsym(sqlite_handle, "sqlite3_errcode");
    }
}
...
static void shim_init(void) {
    ...
    // Optional C++ exception interception for debugging C++ DB exceptions.
