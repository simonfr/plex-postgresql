...
int my_sqlite3_value_type(sqlite3_value *pVal) {
    pg_exception_note_phase("value_type", NULL, pVal, NULL);
    ...
    if (fake && fake->pg_stmt) {
        ...
        if (pg_stmt->result && fake->row_idx >= 0 && fake->row_idx < pg_stmt->num_rows && fake->col_idx < pg_stmt->num_cols) {
            ...
            if (is_null) {
                ...
                result = rust_pg_oid_to_sqlite_type((unsigned int)oid);
            }
...
const unsigned char* my_sqlite3_value_text(sqlite3_value *pVal) {
    pg_exception_note_phase("value_text", NULL, pVal, NULL);
...
int my_sqlite3_value_int(sqlite3_value *pVal) {
    pg_exception_note_phase("value_int", NULL, pVal, NULL);
    ...
    if (fake && fake->pg_stmt) {
        ...
        if (pg_stmt->result && fake->row_idx >= 0 && fake->row_idx < pg_stmt->num_rows && fake->col_idx < pg_stmt->num_cols) {
            ...
            int result = rust_pg_text_to_int(val);
...
sqlite3_int64 my_sqlite3_value_int64(sqlite3_value *pVal) {
    pg_exception_note_phase("value_int64", NULL, pVal, NULL);
    ...
    if (fake && fake->pg_stmt) {
        ...
        if (pg_stmt->result && fake->row_idx >= 0 && fake->row_idx < pg_stmt->num_rows && fake->col_idx < pg_stmt->num_cols) {
            ...
            sqlite3_int64 result = (sqlite3_int64)rust_pg_text_to_int64(val);
            pthread_mutex_unlock(&pg_stmt->mutex);
            return result;
        }
        pthread_mutex_unlock(&pg_stmt->mutex);
...
double my_sqlite3_value_double(sqlite3_value *pVal) {
    ...
    if (fake && fake->pg_stmt) {
        ...
        if (pg_stmt->result && fake->row_idx >= 0 && fake->row_idx < pg_stmt->num_rows && fake->col_idx < pg_stmt->num_cols) {
            ...
            double result = rust_pg_text_to_double(val);
            pthread_mutex_unlock(&pg_stmt->mutex);
            return result;
        }
        pthread_mutex_unlock(&pg_stmt->mutex);
