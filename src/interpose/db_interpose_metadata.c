...
sqlite3_int64 my_sqlite3_last_insert_rowid(sqlite3 *db) {
    ...
    if (!pg_conn) {
        // Do NOT fallback to an arbitrary library connection: that can return
        // another thread/session's lastval() and corrupt caller logic.
        sqlite3_int64 global_rowid = pg_get_global_last_insert_rowid();
        LOG_DEBUG("last_insert_rowid: CALLED db=%p pg_conn=NULL (no exact match, global=%lld)",
                  (void*)db, global_rowid);
        in_interpose_call = 0;
        return global_rowid > 0 ? global_rowid : 0;
    }
    ...
    if (pg_conn->last_insert_rowid > 0) {
        ...
        LOG_DEBUG("last_insert_rowid: using cached connection rowid=%lld", rowid);
        in_interpose_call = 0;
        return rowid;
    }
    ...
    sqlite3_int64 global_rowid = pg_get_global_last_insert_rowid();
    if (global_rowid > 0) {
        LOG_DEBUG("last_insert_rowid: using cached global rowid=%lld", global_rowid);
        in_interpose_call = 0;
        return global_rowid;
    }
    ...
    // Legacy fallback: query PostgreSQL session lastval() only if we don't have
    // cached rowid state (connection/global). This is best-effort and may fail
    // when no INSERT occurred in this session.
...
const char* my_sqlite3_errmsg(sqlite3 *db) {
    ...
    if (in_interpose_call && shim_sqlite3_errmsg) {
        return shim_sqlite3_errmsg(db);
    }
    ...
    const char *msg = NULL;
    if (shim_sqlite3_errmsg) {
        msg = shim_sqlite3_errmsg(db);
...
int my_sqlite3_errcode(sqlite3 *db) {
    ...
    // CRITICAL: Prevent recursion when called from within our shim
    if (in_interpose_call && shim_sqlite3_errcode) {
        return shim_sqlite3_errcode(db);
    }
    ...
    // Only fall through to real SQLite for non-PostgreSQL databases
    int code;
    if (shim_sqlite3_errcode) {
        code = shim_sqlite3_errcode(db);
