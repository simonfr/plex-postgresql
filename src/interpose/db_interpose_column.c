...
static const char* pg_udt_to_sqlite_decltype(const char *udt_name) {
    return rust_pg_udt_to_sqlite_decltype(udt_name);
}
...
static const char* normalize_sqlite_decltype(const char *plex_type) {
    return rust_normalize_sqlite_decltype(plex_type);
}
...
static void trace_badcast_init(void) {
    ...
    rust_load_badcast_config(&trace_badcast_enabled,
                             trace_badcast_idx_file_buf, sizeof(trace_badcast_idx_file_buf),
                             trace_badcast_thread_file_buf, sizeof(trace_badcast_thread_file_buf),
                             trace_badcast_sql_file_buf, sizeof(trace_badcast_sql_file_buf),
                             trace_badcast_col_file_buf, sizeof(trace_badcast_col_file_buf));

    trace_badcast_idx_list = trace_badcast_idx_file_buf[0] ? trace_badcast_idx_file_buf : NULL;
    if (!trace_badcast_idx_list || !trace_badcast_idx_list[0]) trace_badcast_idx_list = "5,6";

    trace_badcast_thread_substr = trace_badcast_thread_file_buf[0] ? trace_badcast_thread_file_buf : NULL;
    if (!trace_badcast_thread_substr || !trace_badcast_thread_substr[0]) {
        // Default to no thread filter.
        trace_badcast_thread_substr = "any";
    }

    trace_badcast_sql_contains = trace_badcast_sql_file_buf[0] ? trace_badcast_sql_file_buf : NULL;
    trace_badcast_col_contains = trace_badcast_col_file_buf[0] ? trace_badcast_col_file_buf : NULL;
}

static int trace_badcast_list_contains_idx(const char *list, int idx) {
    return rust_trace_list_contains_idx(list, idx);
}
...
static int trace_badcast_sql_ok(const pg_stmt_t *pg_stmt) {
    ...
    return rust_trace_list_any_token_in_haystack(trace_badcast_sql_contains, pg_stmt->pg_sql);
}
...
static int trace_badcast_col_ok(const char *col_name) {
    ...
    return rust_trace_list_any_token_in_haystack(trace_badcast_col_contains, col_name);
}
...
static int mask_collection_metadata_type(const pg_stmt_t *pg_stmt, const char *col_name,
    ...
    if (!pg_stmt || !pg_stmt->pg_sql || !col_name || !out) return 0;
    if (!rust_should_mask_collection_metadata_type(pg_stmt->pg_sql, col_name, raw_val)) return 0;
...
static int format_epoch_to_datetime_utc(long long epoch, char *out, size_t out_len) {
    return rust_format_epoch_to_datetime_utc(epoch, out, out_len);
}
...
int my_sqlite3_column_type(sqlite3_stmt *pStmt, int idx) {
    ...
    pg_stmt_t *pg_stmt = pg_find_any_stmt(pStmt);
    const char *dbg_sql = pg_stmt ? (pg_stmt->pg_sql ? pg_stmt->pg_sql : pg_stmt->sql) : NULL;
    sqlite3 *dbg_db = orig_sqlite3_db_handle ? orig_sqlite3_db_handle(pStmt) : NULL;
    pg_exception_note_phase("column_type", dbg_sql, pStmt, dbg_db);
    int trace = 0;
...
const unsigned char* my_sqlite3_column_text(sqlite3_stmt *pStmt, int idx) {
    pg_stmt_t *dbg_stmt = pg_find_any_stmt(pStmt);
    const char *dbg_sql = dbg_stmt ? (dbg_stmt->pg_sql ? dbg_stmt->pg_sql : dbg_stmt->sql) : NULL;
    sqlite3 *dbg_db = orig_sqlite3_db_handle ? orig_sqlite3_db_handle(pStmt) : NULL;
    pg_exception_note_phase("column_text", dbg_sql, pStmt, dbg_db);
    ...
    if (pg_stmt && pg_stmt->is_pg) {
            ...
            if (oid == 23 || oid == 20 || oid == 21) {  // int4, int8, int2
                ...
                if (col_name && (strcmp(col_name, "count") == 0 ||
                    ...
                    if (oid == 20) {  // int8/BIGINT
                        ...
                        // Compatibility: certain Plex code paths parse aggregate
                        // MAX/MIN timestamp-like values as std::tm text. Returning
                        // raw epoch text can trigger "Cannot convert data to std::tm".
                        // If this looks like a timestamp aggregate, provide
                        // SQLite-style datetime text.
                        if ((strcmp(col_name, "max") == 0 || strcmp(col_name, "min") == 0) &&
                            pg_stmt->pg_sql &&
                            (strstr(pg_stmt->pg_sql, "_at") != NULL ||
                             strstr(pg_stmt->pg_sql, "changed_at") != NULL ||
                             strstr(pg_stmt->pg_sql, "updated_at") != NULL ||
                             strstr(pg_stmt->pg_sql, "created_at") != NULL) &&
                            format_epoch_to_datetime_utc(val,
                                                         column_text_buffers[buf_idx],
                                                         TEXT_BUFFER_SIZE)) {
                            LOG_DEBUG("COLUMN_TEXT_AGGREGATE_DATETIME: col='%s' epoch='%s' -> '%s'",
        ...
        if (str_len > 0 && !rust_validate_utf8(source_value, str_len)) {
        ...
        if (rust_rewrite_server_library_uri(source_value, column_text_buffers[buf_idx], TEXT_BUFFER_SIZE)) {
...
const void* my_sqlite3_column_blob(sqlite3_stmt *pStmt, int idx) {
    ...
    if (pg_stmt->cached_result) {
        ...
        if (idx >= 0 && idx < cached->num_cols && row >= 0 && row < cached->num_rows) {
            ...
            if (!crow->is_null[idx] && crow->values[idx]) {
                ...
                result = crow->values[idx];
            }
        }
        goto done;
    }

    if (!pg_stmt->result) goto done;
    if (idx < 0 || idx >= pg_stmt->num_cols || idx >= MAX_PARAMS) goto done;

    int row = pg_stmt->current_row;
    if (row < 0 || row >= pg_stmt->num_rows) goto done;
    if (PQgetisnull(pg_stmt->result, row, idx)) goto done;
    ...
    if (col_type == 17) {  // BYTEA
        int blob_len;
        result = pg_decode_bytea(pg_stmt, row, idx, &blob_len);
        goto done;
    }
    ...
    if (pg_stmt->cached_row == row && pg_stmt->cached_blob[idx]) {
        result = pg_stmt->cached_blob[idx];
        goto done;
    }
...
int my_sqlite3_column_bytes(sqlite3_stmt *pStmt, int idx) {
    ...
    if (!pg_stmt->result) goto done;
    if (idx < 0 || idx >= pg_stmt->num_cols) goto done;
    ...
    result = PQgetlength(pg_stmt->result, row, idx);
...
const char* my_sqlite3_column_name(sqlite3_stmt *pStmt, int idx) {
    ...
    if (pg_stmt && pg_stmt->is_pg) {
        ...
        if (pg_stmt->col_names && idx >= 0 && idx < pg_stmt->num_col_names) {
            result = pg_stmt->col_names[idx];
            LOG_DEBUG("COLUMN_NAME: returning '%s' for idx=%d (from col_names)", result ? result : "NULL", idx);
            use_orig = 0;
            goto done;
        }
        ...
        if (pg_stmt->result && idx >= 0 && idx < pg_stmt->num_cols) {
            result = PQfname(pg_stmt->result, idx);
            LOG_DEBUG("COLUMN_NAME: returning '%s' for idx=%d", result ? result : "NULL", idx);
            use_orig = 0;
            goto done;
        }
    ...
    if (use_orig) {
        result = orig_sqlite3_column_name ? orig_sqlite3_column_name(pStmt, idx) : NULL;
        LOG_DEBUG("COLUMN_NAME: orig returned '%s'", result ? result : "NULL");
    }
    return result;
}
...
const char* my_sqlite3_column_decltype(sqlite3_stmt *pStmt, int idx) {
    ...
    if (!pg_stmt->result || idx < 0 || idx >= pg_stmt->num_cols) {
        ...
        result = "TEXT";  // Safe default that matches SQLITE_TEXT
        goto done_pg;
    }
...

    {
        ...
        int special_case = pg_decltype_special_case(oid, col_name, pg_stmt->pg_sql, table_oid);
...
sqlite3_value* my_sqlite3_column_value(sqlite3_stmt *pStmt, int idx) {
    ...
    const char *dbg_sql = dbg_stmt ? (dbg_stmt->pg_sql ? dbg_stmt->pg_sql : dbg_stmt->sql) : NULL;
    sqlite3 *dbg_db = orig_sqlite3_db_handle ? orig_sqlite3_db_handle(pStmt) : NULL;
    pg_exception_note_phase("column_value", dbg_sql, pStmt, dbg_db);

    pg_stmt_t *pg_stmt = pg_find_any_stmt(pStmt);
    int row = 0;
    int use_orig = 0;
    int build_fake_value = 0;
