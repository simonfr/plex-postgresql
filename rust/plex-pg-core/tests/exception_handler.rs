...
fn cxa_demangle_known_types_if_available() {
    unsafe {
        ...
        let demangled = demangle(name.as_ptr(), std::ptr::null_mut(), std::ptr::null_mut(), &mut status);
        ...
        let demangled2 = demangle(name2.as_ptr(), std::ptr::null_mut(), std::ptr::null_mut(), &mut status2);
