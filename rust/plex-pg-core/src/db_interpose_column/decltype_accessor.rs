...
unsafe fn passthrough_decltype(p_stmt: *mut sqlite3_stmt, idx: c_int) -> *const c_char {
...
pub(super) fn column_decltype_impl(p_stmt: *mut sqlite3_stmt, idx: c_int) -> *const c_char {
    ...
    if pg_stmt.is_null() || unsafe { (*pg_stmt).is_pg == 0 } {
        return unsafe { passthrough_decltype(p_stmt, idx) };
    }
    ...
    if let Some(result) = unsafe { no_result_decltype(pg_stmt, idx) } {
        return result;
    }
