...
fn json_operator_with_parameter() {
    // GAP: JSON ->> operator with $.path not implemented
...
fn json_operator_param_position() {
    // GAP: JSON ->> operator parameter position handling not implemented
    let r =
...
fn json_operator_plex_vad_query() {
    // GAP: Complex Plex query with JSON ->> not implemented
    let r =
