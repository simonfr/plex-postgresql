...
impl LoggerState {
    fn new() -> Self {
        LoggerState {
            file: None,
            path: DEFAULT_LOG_FILE.to_string(),
            is_stdout: false,
            is_stderr: false,
        }
    }
...
fn open_log_file(path: &str) -> Option<File> {
    ...
    match OpenOptions::new().create(true).append(true).open(path) {
        ...
        Err(e) => {
            let _ = writeln!(
                std::io::stderr(),
                "pg_logging: failed to open log file '{}': {}",
                path,
                e
            );
            None
...
pub extern "C" fn rust_logging_init() {
    ...
    INITIALIZED.get_or_init(|| {
        ...
        let path =
            std::env::var("PLEX_PG_LOG_FILE").unwrap_or_else(|_| DEFAULT_LOG_FILE.to_string());
